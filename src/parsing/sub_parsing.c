...
bool	parse_camera(t_data *data, char **splitted)
{
	...
	if (!camera)
		return (perror("Malloc fail\n"), false);
	data->camera = camera;
	if (!(splitted[1] && parse_xyz(&(data->camera->x), &(data->camera->y), &(data->camera->z), splitted[1])))
		return (false);
	if (!(splitted[2] && parse_xyz_norm(&(data->camera->x_orientation), &(data->camera->y_orientation), &(data->camera->z_orientation), splitted[2])))
		return (false);
...
bool	parse_light(t_data *data, char **splitted)
{
	...
	if (!light)
		return(perror("Malloc Fail\n"), false);
	data->light = light;
	if (!(splitted[1] && parse_xyz(&(data->light->x), &(data->light->y), &(data->light->z), splitted[1])))
		return (false);
	if (!(splitted[2] && parse_ratio_light(&(data->light->brightness), splitted[2])))
		return (false);
	if (!(splitted[3] && parse_RGB(&(data->light->r), &(data->light->g), &(data->light->b), splitted[3])))
		return (false);
	if (splitted[3])
		return (false);
	return (true);
